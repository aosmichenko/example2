...
function osmi_is_mobile( $kind = 'any', $return_matched_agent = false ) {
...
class Jetpack_User_Agent_Info {
	...
	static function is_tablet() {
		return ( 0 // never true, but makes it easier to manage our list of tablet conditions
		         || self::is_ipad() || self::is_android_tablet() || self::is_blackberry_tablet() || self::is_kindle_fire() || self::is_MaemoTablet() || self::is_TouchPad() );
	}
	...
	*/
	...
	static function is_opera_mini_dumb() {
		...
		if ( self::is_opera_mini() ) {
			if ( strpos( $ua, 'android' ) !== false || strpos( $ua, 'iphone' ) !== false || strpos( $ua, 'ipod' ) !== false || strpos( $ua, 'ipad' ) !== false || strpos( $ua, 'blackberry' ) !== false ) {
				return false;
			} else {
				return true;
			}
			...
			if ( $pos_torch !== false ) {
				...
				if ( preg_match( '#Version\/([\d\.]+)#i', $agent, $matches ) ) {
					...
					if ( is_array( $version_num ) === false || count( $version_num ) <= 1 ) {
						return 'blackberry-6';
					} //not a BB device that match our rule.
					else {
